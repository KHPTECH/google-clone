...
function SearchReasults({ results }) {
  return (
        ...
        <div key={results.link} className="max-w-xl mb-8">
          <div className="group">
            <a href={results.link} className=" text-sml">
                <h5 className="truncate ">{results.formattedUrl}</h5>
              
            </a>
            <a href={results.link}>
              <h2 className="text-xl truncate text-blue-800 font-medium group-hover:underline">{results.title}</h2>
            </a>
          </div>
